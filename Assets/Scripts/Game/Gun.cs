...
public class Gun : MonoBehaviour
{
    ...
    private float firstBulletDirection;
    ...
    public void ShootGun(Transform director)
    {
        ...
        if (shotgun)
        {
            ...
            {
                // math andgle for every bullet
                float angleOffset = Mathf.Lerp(-shotgunSpread / 2, shotgunSpread / 2, (float)i / (shotgunBullets - 1));
        
                // create bullet rotation
                Quaternion bulletRotation = Quaternion.Euler(director.eulerAngles + new Vector3(0, 0, angleOffset));
